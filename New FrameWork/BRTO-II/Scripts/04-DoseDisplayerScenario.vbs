...
Class TestTool
   ...
   Public Sub New()
        ...
        Dim brtoDoseDisplayerDataset As New BRTODoseDisplayerDataset

        brtoDoseDisplayerDataset.ShowDialog()

        Dim brtoGeometricPlannerDataset As New BRTOGeometricPlannerDataset

        if not brtoChooseStructureSetDialog.NormalStructureSet
            brtoDoseDisplayerDataset.btnSelectStructureSet.Text = "High-Resolution RT Struct file"
        end if
        ...
        If Not (String.IsNullOrEmpty(brtoDoseDisplayerDataset.tbSelectedImageSet.Text)) Then

            Dim dirInfo As IO.DirectoryInfo = New IO.DirectoryInfo(brtoDoseDisplayerDataset.tbSelectedImageSet.Text)
            ...
            file = new IO.FileInfo(brtoDoseDisplayerDataset.tbSelectedStructset.Text)
            file.CopyTo(path + file.Name, True)
            STRUCTNode.InnerText = file.Name
            structureSetFound = true

            file = new IO.FileInfo(brtoDoseDisplayerDataset.tbSelectedRTPlan.Text)
            file.CopyTo(path + file.Name, True)
            DOSIMETRICPLANNode.InnerText = file.Name

            file = new IO.FileInfo(brtoDoseDisplayerDataset.tbSelectedRTDose.Text)
            file.CopyTo(path + file.Name, True)
   ...
   End Sub
    ...
    If(imagesFound and structureSetFound and allImagesSameFOR)
    	'run scenario
		m_scenarioRunner.RunScenario(New DoseDisplayerScenario(normalStructureSet))
