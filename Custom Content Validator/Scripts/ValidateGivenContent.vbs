...
Inherits TestToolBase
    ...
    Protected Overrides Sub Execute()
        ...
        'run scenario
        m_scenarioRunner.RunScenario(New ValidateGivenContent(m_mediaSession))
    End Sub
...
#If DVT_INTERPRETS_SCRIPT Then
		...
		mediaSession =  Dvtk.Sessions.MediaSession.LoadFromFile("..\Sessionfiles\TestTool media - ContentValidation.ses")
        sessionFileName = "..\Sessionfiles\TestTool script - ContentValidation.ses"
...
#Else
        mediaSession =  Dvtk.Sessions.MediaSession.LoadFromFile("..\..\..\..\DVtkProject\InitialContentValidationFiles\Sessionfiles\TestTool media - ContentValidation - Test.ses")
        sessionFileName = "..\..\..\..\DVtkProject\InitialContentValidationFiles\Sessionfiles\TestTool script - ContentValidation - Test.ses"
#End If
...
'start the test tool thread
        ...
        theDvtThreadManager.WaitForCompletionThreads()
         ...
         If( IO.File.Exists("..\ResultDicomValidation\Summary_ContentValidation.html"))
          Dim line As String

                    ' Create new StreamReader instance with Using block.
                    Using reader As IO.StreamReader = New IO.StreamReader("..\ResultDicomValidation\Summary_ContentValidation.html", True)
                        ' Read whole file
                        line = reader.ReadToEnd
                    End Using

            Dim newLine As String = line.Replace("&gt;", ">")
            theTestTool.WriteHtml(newLine, False, True)
